    ...
    const std::vector<uptr>& actorPtrs)
{
    ...
    for (i32 index = 0; index < actorCount; ++index)
    {
        ...
        if (needBodyInstance)
        {
            ...
            if (bodyInstanceOffset >= 0)
            {
                ...
                std::cerr << std::format(
                    "[xrd][Chaos] PrimComp::BodyInstance = "
                    "+0x{:X} (actor-walk)\n",
                    bodyInstanceOffset);
            }
        }
    ...
    {
        std::cerr
            << "[xrd][Chaos] PrimComp::BodyInstance 未找到\n";
    }
    if (needAggregateGeometry)
    {
        std::cerr
            << "[xrd][Chaos] BodySetup::AggGeom 未找到\n";
    }
    if (needBodyInstanceSize)
    {
        std::cerr
            << "[xrd][Chaos] FBodyInstance 结构大小未知\n";
    }
}
