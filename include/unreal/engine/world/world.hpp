#pragma once
// Unreal-eXternalrEsolve - UWorld / Actor / Pawn 访问
// 游戏世界遍历的高层 API
...
namespace detail
{
    ...
    inline void DiscoverWorldChainOffsets()
    {
        ...
        {
            std::cerr << "[unreal] ULocalPlayer类: 0x" << std::hex << lpClass << std::dec << "\n";
            std::cerr << "[unreal] 尝试反射查找PlayerController属性...\n";
            ctx.off.ULocalPlayer_PlayerController = GetPropertyOffsetByName(lpClass, "PlayerController");
            std::cerr << "[unreal] PlayerController偏移: " << ctx.off.ULocalPlayer_PlayerController << "\n";
        }

        if (ctx.off.ULocalPlayer_PlayerController == -1)
        {
            std::cerr << "[unreal] ERROR: 无法找到ULocalPlayer::PlayerController属性\n";
            return;
        }
        ...
        {
            std::cerr << "[unreal] PlayerController实例无效: pc=0x" << std::hex << pc << std::dec << "\n";
            return;
        }

        std::cerr << "[unreal] PlayerController实例: 0x" << std::hex << pc << std::dec << "\n";

        uptr pcClass = GetObjectClass(pc);
        if (!pcClass)
        {
            std::cerr << "[unreal] 无法获取PlayerController类\n";
            return;
        }

        std::cerr << "[unreal] PlayerController类: 0x" << std::hex << pcClass << std::dec << "\n";

        if (ctx.off.APlayerController_Pawn == -1)
        {
            std::cerr << "[unreal] 尝试反射查找Pawn属性...\n";
            i32 pawnOff = GetPropertyOffsetByName(pcClass, "Pawn");
            std::cerr << "[unreal] Pawn偏移: " << pawnOff << "\n";
            if (pawnOff == -1)
            {
                std::cerr << "[unreal] 尝试反射查找AcknowledgedPawn属性...\n";
                pawnOff = GetPropertyOffsetByName(pcClass, "AcknowledgedPawn");
                std::cerr << "[unreal] AcknowledgedPawn偏移: " << pawnOff << "\n";
            }
            ctx.off.APlayerController_Pawn = pawnOff;
        }
        if (ctx.off.APlayerController_PlayerCameraManager == -1)
        {
            std::cerr << "[unreal] 尝试反射查找PlayerCameraManager属性...\n";
            i32 camOff = GetPropertyOffsetByName(pcClass, "PlayerCameraManager");
            std::cerr << "[unreal] PlayerCameraManager偏移: " << camOff << "\n";
            ctx.off.APlayerController_PlayerCameraManager = camOff;
        }

        std::cerr << "[unreal] World链偏移（延迟发现）:\n";
