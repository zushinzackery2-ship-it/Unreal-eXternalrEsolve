...
namespace detail
{
    ...
    inline StructMemberRange GetSkinnedMeshComponentMemberRange()
    {
        ...
        if (!cls)
        {
            std::cerr << "[unreal] 未找到类 SkinnedMeshComponent，无法限定骨骼偏移范围\n";
            return cachedRange;
        }
        ...
        {
            std::cerr << "[unreal] SkinnedMeshComponent 成员范围无效: begin=0x"
        ...
        std::cerr << "[unreal] USkinnedMeshComponent 自身成员范围: [0x"
    ...
    inline i32 DiscoverComponentSpaceTransformsOffset(uptr meshComponent, bool isDouble)
    {
        ...
        {
            std::cerr << "[unreal] ComponentSpaceTransforms 候选: "
                      << candidateLog.str()
