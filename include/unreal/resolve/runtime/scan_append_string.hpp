#pragma once
// Unreal-eXternalrEsolve - AppendString 扫描器
    ...
    UEOffsets& off)
{
    ...
    if (forwardStringRef)
    {
        std::cerr << "[unreal] ForwardShadingQuality_ xref RVA=0x"
        ...
        {
            std::cerr << "[unreal] ForwardShadingQuality_ 主签名未命中\n";
        }
        ...
        if (!primaryAppendString || bPrimaryMayBeInlineOverlap)
        {
            ...
            if (!primaryAppendString)
            {
                std::cerr << "[unreal] ForwardShadingQuality_ 内联回退未命中\n";
            }
        }
    ...
    {
        std::cerr << "[unreal] 未找到 ForwardShadingQuality_ 字符串引用\n";
    }
    ...
    if (boneStringRef)
    {
        ...
        std::cerr << "[unreal] Bone 字符串回退未命中\n";
    }
    else
    {
        std::cerr << "[unreal] 未找到 Bone 字符串引用\n";
    }
    ...
    {
        std::cerr << "[unreal] Conv_NameToString Exec RVA=0x"
        ...
        std::cerr << "[unreal] Conv_NameToString 回退未命中\n";
    }
    else
    {
        std::cerr << "[unreal] 未找到 Conv_NameToString Exec\n";
    }

    std::cerr << "[unreal] AppendString 未找到\n";
    return false;
}
