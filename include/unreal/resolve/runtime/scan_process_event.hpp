#pragma once
// Unreal-eXternalrEsolve - ProcessEvent 扫描器
    ...
    UEOffsets& off)
{
    ...
    {
        std::cerr << "[unreal] FunctionFlags 未知，跳过 ProcessEvent 扫描\n";
        return false;
    }
    ...
    for (i32 idx = 0; idx < 200; ++idx)
    {
        ...
        if (found1 && found2)
        {
            ...
            std::cerr << "[unreal] ProcessEvent 找到: VTable["
                      << idx << "] RVA=0x"
    ...
    std::cerr << "[unreal] ProcessEvent 未找到\n";
    return false;
}
