#pragma once
// Unreal-eXternalrEsolve - GNames 扫描器
    ...
    bool& outIsNamePool)
{
    ...
    if (!dataSection)
    {
        std::cerr << "[unreal] .data 段未找到（GNames 扫描）\n";
        return false;
    }
    ...
    {
        outIsNamePool = true;
        std::cerr << "[unreal] GNames (FNamePool) 找到: 0x"
                  << std::hex << outGNames
                  << " (ctor)\n" << std::dec;
        return true;
    }
    ...
    {
        outIsNamePool = false;
        std::cerr << "[unreal] GNames (TNameEntryArray) 找到: 0x"
                  << std::hex << outGNames
                  << " (.data fallback)\n" << std::dec;
        return true;
    }
    ...
    UEOffsets& off)
{
    ...
    if (numChunks <= 0)
    {
        std::cerr << "[unreal] FNamePool NumChunks 读取失败，BlockBits 保持默认\n";
        return;
    }
    ...
    {
        std::cerr << "[unreal] FNamePoolBlockBits 探测失败，保持默认\n";
        return;
    }

    off.FNamePoolBlockBits = blockBits;
    std::cerr << "[unreal] FNamePoolBlockBits: 0x"
