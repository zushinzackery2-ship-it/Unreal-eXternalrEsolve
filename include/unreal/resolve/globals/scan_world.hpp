#pragma once
// Unreal-eXternalrEsolve - GWorld 扫描器
    ...
    uptr& outGWorld)
{
    ...
    {
        std::cerr << "[unreal] GWorld 未找到（无 World 实例）\n";
        return false;
    }
    ...
    if (results.empty())
    {
        std::cerr << "[unreal] GWorld 未找到（.data 中无指向 World 实例的指针）\n";
        return false;
    }
    ...
    {
        // 多于 2 个结果，取第一个
        std::cerr << "[unreal] 检测到 " << results.size() << " 个 GWorld 候选\n";
        outGWorld = results[0];
    }

    std::cerr << "[unreal] GWorld 找到: 0x" << std::hex << outGWorld << std::dec << "\n";
    return true;
}
