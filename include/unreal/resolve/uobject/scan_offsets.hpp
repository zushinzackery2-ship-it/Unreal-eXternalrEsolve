#pragma once
// Unreal-eXternalrEsolve - UObject 偏移发现
...
inline bool DiscoverUObjectOffsets(const IMemoryAccessor& mem, UEOffsets& off)
{
    ...
    if (totalObjects <= 0)
    {
        std::cerr << "[unreal] 对象数量为 0\n";
        return false;
    }

    std::cerr << "[unreal] 对象总数: " << totalObjects << "\n";
    ...
    if (samples.size() < 10)
    {
        std::cerr << "[unreal] 有效对象太少: " << samples.size() << "\n";
        return false;
    }

    std::cerr << "[unreal] 采样 " << samples.size() << " 个对象用于偏移发现\n";
    ...
    // 选命中率最高且超过 50% 的候选（GObjects 槽位可能有已销毁对象）
    {
        ...
        for (i32 classOff : {0x10, 0x18, 0x08})
        {
            ...
            std::cerr << "[unreal] Class 候选 +0x" << std::hex << classOff << std::dec
        ...
        if (bestOff != -1 && bestCount > (int)samples.size() * 50 / 100)
        {
            ...
            std::cerr << "[unreal] UObject::Class +0x" << std::hex << bestOff
    ...
    {
        std::cerr << "[unreal] 未找到 UObject::Class 偏移\n";
        return false;
    }
    ...
    for (i32 idxOff : {0x0C, 0x08, 0x04})
    {
        ...
        std::cerr << "[unreal] Index 候选 +0x" << std::hex << idxOff << std::dec
                  << " 匹配: " << matchCount << "/100\n";
        if (matchCount > 20)
        {
            off.UObject_Index = idxOff;
            std::cerr << "[unreal] UObject::Index +0x" << std::hex << idxOff
    ...
    for (i32 flagOff : {0x08, 0x04, 0x0C})
    {
        ...
        if (validCount > 0)
        {
            ...
            std::cerr << "[unreal] UObject::Flags +0x" << std::hex << flagOff << std::dec << "\n";
            break;
        }
    }
    ...
    for (i32 nameOff : {0x18, 0x20, 0x10, 0x28})
    {
        ...
        std::cerr << "[unreal] Name 候选 +0x" << std::hex << nameOff << std::dec
                  << " 命中: " << validCount << "/50\n";
        if (validCount > 15)
        {
            off.UObject_Name = nameOff;
            std::cerr << "[unreal] UObject::Name +0x" << std::hex << nameOff
    ...
    for (i32 outerOff : {0x20, 0x28, 0x30, 0x18})
    {
        ...
        if (validOrNull > (int)samples.size() * 70 / 100)
        {
            ...
            std::cerr << "[unreal] UObject::Outer +0x" << std::hex << outerOff << std::dec << "\n";
            break;
        }
    }
