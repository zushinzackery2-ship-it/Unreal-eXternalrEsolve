#pragma once
// Unreal-eXternalrEsolve - UFunction 偏移发现
    ...
    UEOffsets& off)
{
    ...
    if (found != -1)
    {
        ...
        std::cerr << "[unreal] UFunction::FunctionFlags +0x"
    ...
    UEOffsets& off)
{
    ...
    {
        std::cerr << "[unreal] UFunction 对象太少\n";
        return false;
    }
    ...
    UEOffsets& off)
{
    ...
    if (wasInputKeyJustPressed && toggleSpeaking && switchLevelOrFov)
    {
        for (i32 testOff = 0x30; testOff <= 0x140; testOff += static_cast<i32>(sizeof(uptr)))
        {
            ...
            std::cerr << "[unreal] UFunction::ExecFunction +0x"
