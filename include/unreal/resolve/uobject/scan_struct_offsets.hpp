#pragma once
// Unreal-eXternalrEsolve - UStruct 偏移发现
    ...
    UEOffsets& off)
{
    ...
    {
        std::cerr << "[unreal] 未找到 Class/Struct 元类对象\n";
        return false;
    }
    ...
    if (superOff == -1)
    {
        std::cerr << "[unreal] SuperStruct 偏移未找到\n";
        return false;
    }
    ...
    if (fieldObj)
    {
        ...
        {
            std::cerr << "[unreal] SuperStruct 交叉验证失败\n";
        }
    }

    off.UStruct_SuperStruct = superOff;
    std::cerr << "[unreal] UStruct::SuperStruct +0x"
         ...
         childOff += 8)
    {
            ...
            validCount > sampleCount * 80 / 100)
        {
            ...
            std::cerr << "[unreal] UStruct::Children +0x"
    ...
    if (off.UStruct_Children != -1)
    {
        ...
        if (verified)
        {
            ...
            std::cerr << "[unreal] UStruct::ChildProperties +0x"
                      << std::hex << cpOff << std::dec << "\n";
        }
        else
        {
            // 回退：不使用 FProperty
            std::cerr << "[unreal] ChildProperties 未验证，"
                      << "回退到 UField 模式\n";
        }
    }
    ...
    {
        std::cerr << "[unreal] UStruct::Size +0x"
