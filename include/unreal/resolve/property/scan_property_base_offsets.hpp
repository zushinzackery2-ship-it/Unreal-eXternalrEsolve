#pragma once
// Unreal-eXternalrEsolve - Property 基础偏移发现
    ...
    UEOffsets& off)
{
    ...
    if (!propA || !propD)
    {
        std::cerr << "[unreal] 未找到 Guid.A/D 属性\n";
        return false;
    }
    ...
    for (i32 testOff = 0x30; testOff <= 0x60; testOff += 4)
    {
        ...
        if (valA == 4 && valD == 4)
        {
            ...
            std::cerr << "[unreal] Property::ElementSize +0x"
    ...
    UEOffsets& off)
{
    ...
    for (i32 testOff = lo; testOff <= hi; testOff += 4)
    {
        ...
        if (valA == 1 && valC == 1)
        {
            ...
            std::cerr << "[unreal] Property::ArrayDim +0x"
                      << std::hex << testOff << std::dec << "\n";
            return true;
        }
    }
    ...
    UEOffsets& off)
{
    ...
    for (i32 testOff = 0x30; testOff <= 0x60; testOff += 4)
    {
        ...
        if (valA == 0x00 && valB == 0x04 && valC == 0x08)
        {
            ...
            std::cerr << "[unreal] Property::Offset_Internal +0x"
                      << std::hex << testOff << std::dec << "\n";
            return true;
        }
    }
    ...
    UEOffsets& off)
{
    ...
    for (i32 testOff = searchStart; testOff <= searchEnd; testOff += 8)
    {
        ...
        if (flagsA != 0 && flagsA == flagsD)
        {
            ...
            std::cerr << "[unreal] Property::PropertyFlags +0x"
                      << std::hex << testOff << std::dec << "\n";
            return true;
        }
    }
