#pragma once
// Unreal-eXternalrEsolve - AutoInit 事务式生命周期状态机
    ...
    bool Run()
    {
        ...
        while (true)
        {
            ...
            case AutoInitStage::WaitingForRetry:
                ...
                std::cerr << "[unreal] === Init 第 " << attempt << " 轮 ===\n";
                stateMachine.TransitionTo(AutoInitStage::ScanningGlobals);
                break;
            ...
            case AutoInitStage::Completing:
                ...
                std::cerr << "[unreal] === " << configuration.modeTag
                          << " 完成 ===\n";
...
private:
    ...
    {
        std::cerr << "[unreal] === Unreal-eXternalrEsolve "
                  << configuration.modeTag << " ===\n";
    }

    void ScheduleRetry(const char* reason)
    {
        std::cerr << "[unreal] " << reason << "，"
