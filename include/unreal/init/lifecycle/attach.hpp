#pragma once
// Unreal-eXternalrEsolve - AutoInit 进程与内存后端附加
...
inline u32 FindAutoInitTargetProcess(const wchar_t* processName)
{
    ...
    if (pid == 0)
    {
        std::cerr << "[unreal] 进程未找到\n";
    }
    else
    {
        std::cerr << "[unreal] PID: " << pid << "\n";
    }
...
inline bool AttachWinApiProcess(Context& ctx)
{
    ...
    {
        std::cerr << "[unreal] 打开进程失败（尝试以管理员运行）\n";
        return false;
    }
    ...
    const wchar_t* processName)
{
    ...
    {
        std::cerr << "[unreal] 共享内存初始化失败（确认 ReiVM 驱动已加载）\n";
        return false;
    }
