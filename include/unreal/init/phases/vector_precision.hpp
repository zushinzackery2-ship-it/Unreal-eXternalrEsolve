#pragma once
// Unreal-eXternalrEsolve - FVector 精度发现阶段
...
inline InitStepResult RunVectorPrecisionDiscoveryPhase()
{
    ...
    for (i32 index = 0; index < actorCount; ++index)
    {
        ...
        if (elementSize == 24)
        {
            ...
            detected = true;
            std::cerr << "[unreal] FVector 精度: double (UE5) "
        ...
        {
            detected = true;
            std::cerr << "[unreal] FVector 精度: float (UE4) "
    ...
    if (!detected)
    {
        std::cerr << "[unreal] 无法检测 FVector 精度，默认 float\n";
    }
