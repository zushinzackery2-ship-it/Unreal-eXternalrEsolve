#pragma once
// Unreal-eXternalrEsolve - SDK 导出主入口
...
inline bool DumpCppSdk(const std::wstring& outputPath)
{
    ...
    std::cerr << "[unreal] 开始导出 Dumper7 品质 C++ SDK...\n";
    ...
    std::cerr << "[unreal] 找到 " << entries.size()
              << " 个类/结构体, "
              << allEnums.size() << " 个枚举, "
              << pkgMap.size() << " 个包 ("
              << elapsed() << "s)\n";
    ...
    std::cerr << "[unreal] 查找表填充完成 (" << elapsed() << "s)\n";
    std::cerr.flush();

    // 预热属性缓存
    std::cerr << "[unreal] 预热缓存...\n";
    std::cerr.flush();
    ...
    // 当子类属性侵入父类尾部 padding 时，需要用 #pragma pack(push, 0x1) + alignas
    {
        ...
        std::cerr << "[unreal] 尾部 padding 重用检测完成 ("
            << elapsed() << "s)\n";
        std::cerr.flush();
    }
    ...
    detail::GenerateAuxiliaryFiles(cppSdkDir, collisionNsMap,
        entries);
    std::cerr << "[unreal] 辅助文件生成完成 (" << elapsed() << "s)\n";
    std::cerr.flush();

    // 收集跨包依赖（key 为包的 GObjects 索引）
    std::cerr << "[unreal] 收集跨包依赖 ("
        << entries.size() << " 条目)...\n";
    std::cerr.flush();
    ...
    std::cerr << "[unreal] C++ SDK 导出完成: " << pkgCount
              << " 个包, " << sdkIncludes.size()
              << " 个文件\n";
    return true;
}
